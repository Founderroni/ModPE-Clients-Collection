var ctx = com.mojang.minecraftpe.MainActivity.currentMainActivity.get(), Button = android.widget.Button, LinearLayout = android.widget.LinearLayout, RelativeLayout = android.widget.RelativeLayout, PopupWindow = android.widget.PopupWindow, InputType = android.text.InputType,ScrollView = android.widget.ScrollView, KeyEvent = android.view.KeyEvent,TextView = android.widget.TextView, CheckBox = android.widget.CheckBox, LayoutParams = android.view.ViewGroup.LayoutParams, Typeface = android.graphics.Typeface, Switch = android.widget.Switch, BufferedReader=java.io.BufferedReader,InputStreamReader=java.io.InputStreamReader,Toast = android.widget.Toast, Runnable = java.lang.Runnable, URL=java.net.URL,nanoTime = java.lang.System.nanoTime(), Html = android.text.Html,ClipboardManager = android.content.ClipboardManager, SystemClock = android.os.SystemClock, AlarmManager = android.app.AlarmManager, AudioManager = android.media.AudioManager, ToneGenerator = android.media.ToneGenerator, Handler = android.os.Handler, Looper = android.os.Looper, System = java.lang.System, DownloadManager = android.app.DownloadManager, Dialog = android.app.Dialog, ConnectivityManager = android.net.ConnectivityManager, Notification = android.app.Notification, PendingIntent = android.app.PendingIntent, View = android.view.View, ColorDrawable = android.graphics.drawable.ColorDrawable, Color = android.graphics.Color, Gravity = android.view.Gravity, PorterDuff = android.graphics.PorterDuff, Intent = android.content.Intent, DialogInterface = android.content.DialogInterface, DataOutputStream = java.io.DataOutputStream, BufferedReader = java.io.BufferedReader, Thread = java.lang.Thread, TypedValue = android.util.TypedValue, Byte = java.lang.Byte, Context = android.content.Context, Handler = android.os.Handler, TextWatcher = android.text.TextWatcher, FileWriter = java.io.FileWriter, InputStreamReader = java.io.InputStreamReader, SpannableStringBuilder = android.text.SpannableStringBuilder, ImageSpan = android.text.style.ImageSpan, Spannable = android.text.Spannable, StringBuffer = java.lang.StringBuffer, Calendar = java.util.Calendar, Html = android.text.Html, ZipFile = java.util.zip.ZipFile, ByteBuffer = java.nio.ByteBuffer, MediaPlayer = android.media.MediaPlayer, String = java.lang.String, StringBuilder = java.lang.StringBuilder, printWriter = java.io.printWriter,Uri = android.net.Uri, activity = com.mojang.minecraftpe.MainActivity.currentMainActivity.get(), TranslateAnimation = android.view.animation.TranslateAnimation, Animation = android.view.animation.Animation, RotateAnimation = android.view.animation.RotateAnimation, DecelerateInterpolator = android.view.animation.DecelerateInterpolator, LinearInterpolator = android.view.animation.LinearInterpolator, Environment = android.os.Environment, FileInputStream = java.io.FileInputStream, SharedPreferences = android.content.SharedPreferences, numbers = android.text.InputType.TYPE_CLASS_NUMBER, number = android.text.InputType.TYPE_NUMBER_FLAG_SIGNED, ImageView = android.widget.ImageView, EditText = android.widget.EditText, CompoundButton = android.widget.CompoundButton, SeekBar = android.widget.SeekBar, Build = android.os.Build, ProgressBar = android.widget.ProgressBar, ToggleButton = android.widget.ToggleButton, FrameLayout = android.widget.FrameLayout, BitmapFactory = android.graphics.BitmapFactory, ViewGroup = android.view.ViewGroup, GradientDrawable = android.graphics.drawable.GradientDrawable, Bitmap = android.graphics.Bitmap, Canvas = android.graphics.Canvas, Paint = android.graphics.Paint, Path = android.graphics.Path, LinearGradient = android.graphics.LinearGradient, Shader = android.graphics.Shader, MotionEvent = android.view.MotionEvent, BitmapDrawable = android.graphics.drawable.BitmapDrawable, StateListDrawable = android.graphics.drawable.StateListDrawable, ViewTreeObserver = android.view.ViewTreeObserver, ClipDrawable = android.graphics.drawable.ClipDrawable, LightingColorFilter = android.graphics.LightingColorFilter, WindowManager = android.view.WindowManager, DisplayMetrics = android.util.DisplayMetrics, ScriptManager = net.zhuoweizhang.mcpelauncher.ScriptManager, File = java.io.File, FileOutputStream = java.io.FileOutputStream, Base64 = android.util.Base64,
	...
	pvpTab = [
		...
		{name:"Auto Hit",optionSensitive:true,type:"switch",keystate:false,setting:"CPS:35;",delayedStart:true,state:false,rp:"delayedthread",call:"if(!needR){if(Entity.getEntityTypeId(Player.getPointedEntity()) !== 0)inst.sendKeyDownUpSync(android.view.KeyEvent.KEYCODE_Q);}"}
	],
	potionTab = [
	...
	movementTab = [
		...
		{name:"NoFallDamage",rp:"",type:"switch",state:false,keystate:false,setting:"sensitivity:-0.5;",call:"Entity.getVelY(getPlayerEnt())<parseFloat(movementTab[1].setting.split(';')[0].split(':')[1])?setVelY(Player.getEntity(), 0.00000):''"},
	...
	exploitTab = [
		...
		{name:"Spam",rp:"threaded",setting:"message:instinctmods.com;speed:20;",type:"switch",state:false,call:"if(reservedSpam!==0){reservedSpam--}if(reservedSpam==0){Server.sendChat('['+Math.floor(Math.random() * 1000)+'] '+exploitTab[11].setting.split(';')[0].split(':')[1]);reservedSpam=parseInt(exploitTab[11].setting.split(';')[1].split(':')[1])}"},
		{name:"RGB",rp:"t10",rgbtick:"1",setting:"speed:12;",state:true,type:"switch",call:"if(exploitTab[12].rgbtick==0){if(r > 0 && b == 0){ r=r-5; g=g+5; menuRed=r; menuGreen=g; menuBlue=b; } if(g > 0 && r == 0){ g=g-5; b=b+5; menuRed=r; menuGreen=g; menuBlue=b; } if(b > 0 && g == 0){ r=r+5; b=b-5; menuRed=r; menuGreen=g; menuBlue=b; }exploitTab[12].rgbtick=getSetting(exploitTab,'RGB','speed');try{ for(var i=0;i<listar.length;i++){ if(listar[i]!=undefined&&menuOpen){ ctx.runOnUiThread( new java.lang.Runnable( { run:function(){ try{ var rgb=Color.parseColor(rgbToHex(menuRed,menuGreen,menuBlue));listar[i].getBackground().setColor(rgb);var colorStateList = android.content.res.ColorStateList.valueOf(rgb);searchb.setBackgroundTintList(colorStateList);}catch(err){}}}));  } } }catch(e){} }else{ exploitTab[12].rgbtick-- } "},
		...
		{name:"Auto Click",optionSensitive:true,type:"switch",keystate:false,setting:"CPS:35;",delayedStart:true,state:false,rp:"delayedthread",call:"if(!needR){inst.sendKeyDownUpSync(android.view.KeyEvent.KEYCODE_Q);}"},
		{name:"F5",optionSensitive:true,type:"button",disablerp:false,rp:"kb",style:"white",keystate:false,call:"keyevent('f5')"},
		{name:"Twerk",optionSensitive:true,type:"switch",keystate:false,delayedStart:true,state:false,rp:"threaded",call:"inst.sendKeyDownUpSync(android.view.KeyEvent.KEYCODE_SHIFT_LEFT);"},
	...
	friendTab = [
		...
		{name:"Add Pointed",setting:"Name: ;",style:"white",type:"button",call:" if(Entity.getNameTag(Player.getPointedEntity())!=null){ print('Added '+Entity.getNameTag(Player.getPointedEntity())); friendTab[objectLength(friendTab)]={name:Entity.getNameTag(Player.getPointedEntity()),state:true};ctx.runOnUiThread(new Runnable({ run: function() { try{ friendview.removeAllViews(); for(var i=0;i<friendTab.length;i++){ 	 friendview.addView(createItem(i,friendTab)); } listar[6].setText(tohtml('Friends ('+(friendTab.length-2)+')',true)); }catch(e){} }})); }else{ print('Not pointed at a player, or not close enough') }; "},
		{name:"Shark",state:true}
	],	
	waypointTab = [
	...
	favoriteTab = [
		{name:"Add/Edit Favorite",id:8,style:"white",type:"button",open:false,view:"return favoriteview",menu:false,call:"favoriteTab[0].open=true;runKeybinds(true);favoriteTab[0].menu=false;buttonview.removeAllViews();survivalview.removeAllViews(); pvpview.removeAllViews(); potionview.removeAllViews(); movementview.removeAllViews(); exploitview.removeAllViews();friendview.removeAllViews();favoriteview.removeAllViews(); waypointview.removeAllViews();for(var i=0;i<survivalTab.length;i++){ if (favoriteTab.find( e => e.name == survivalTab[i].name )!==undefined) { survivalview.addView(createItem(i,survivalTab,false,'on')); }else{ survivalview.addView(createItem(i,survivalTab,false,'off')); } } for(var i=0;i<pvpTab.length;i++){ if (favoriteTab.find( e => e.name == pvpTab[i].name )!==undefined) { pvpview.addView(createItem(i,pvpTab,false,'on')); }else{ pvpview.addView(createItem(i,pvpTab,false,'off')); } } for(var i=0;i<potionTab.length;i++){ if (favoriteTab.find( e => e.name == potionTab[i].name )!==undefined) { potionview.addView(createItem(i,potionTab,false,'on')); }else{ potionview.addView(createItem(i,potionTab,false,'off')); } } for(var i=0;i<movementTab.length;i++){ if (favoriteTab.find( e => e.name == movementTab[i].name )!==undefined) { movementview.addView(createItem(i,movementTab,false,'on')); }else{ movementview.addView(createItem(i,movementTab,false,'off')); } } for(var i=0;i<exploitTab.length;i++){ if (favoriteTab.find( e => e.name == exploitTab[i].name )!==undefined) { exploitview.addView(createItem(i,exploitTab,false,'on')); }else{ exploitview.addView(createItem(i,exploitTab,false,'off')); } } listar[0].setLayoutParams(new LinearLayout.LayoutParams(dip2px(0), dip2px(0))); listar[1].setLayoutParams(new LinearLayout.LayoutParams(dip2px(0), dip2px(0))); listar[2].setLayoutParams(new LinearLayout.LayoutParams(dip2px(0), dip2px(0))); listar[8].setLayoutParams(new LinearLayout.LayoutParams(dip2px(0), dip2px(0))); listar[3].setLayoutParams(new LinearLayout.LayoutParams(dip2px(0), dip2px(0))); listar[4].setLayoutParams(new LinearLayout.LayoutParams(dip2px(0), dip2px(0))); listar[6].setLayoutParams(new LinearLayout.LayoutParams(dip2px(0), dip2px(0)));closemenu.setText('Finish');listar[7].setLayoutParams(new LinearLayout.LayoutParams(dip2px(0), dip2px(0))); "} 		
	],
	ESP = {
			...
			drawLine: function (gl, x, y, z, x2, y2, z2) {
				...
				gl.glColor4f(menuRed/255, menuGreen/255, menuBlue/255, 0.7);
				}else{
...
/*{name:"Name",type:"button",state:false,call:""}*/
	...
	_0=0,_1=1,_2=2,_3=3,_4=4,_5=5,_6=6,Second=1000,
	rainbow = ["#C7FF0000","#C7FF7F00","#C7FFFF00","#C700FF00","#C70000FF","#C74B0082","#C79400D3"],
	rgbsubtick = 0,
	tempkb = new Array(1000),kbgui = new Array(1000),tempkbState = new Array(1000),listar = new Array(5),r=255,g=0,b=0,r_=255,g_=0,b_=0,sr=250,sg=0,sb=5,horizontalmainlist,hostlayoutlist,
	...
	var disabledBox = new GradientDrawable(); disabledBox.setColor(Color.RED); disabledBox.setAlpha(160);
	...
	var closestyle = new GradientDrawable(); closestyle.setColor(defB); closestyle.setStroke(dip2px(1), defB); closestyle.setAlpha(160);
	var closestyle1 = new GradientDrawable(); closestyle1.setColor(defB); closestyle1.setStroke(dip2px(1), defB); closestyle1.setAlpha(160);
	var editstyle = new GradientDrawable(); editstyle.setColor(defB); editstyle.setStroke(dip2px(1), defB); editstyle.setAlpha(160);
	...
	var backgroundstyle1 = new GradientDrawable(); backgroundstyle1.setColor(defC); backgroundstyle1.setStroke(dip2px(1), defB); backgroundstyle1.setAlpha(200);
	var backgroundstyle2 = new GradientDrawable(); backgroundstyle2.setColor(defC); backgroundstyle2.setStroke(dip2px(1), defB); backgroundstyle2.setAlpha(200);
	var backgroundstyle3 = new GradientDrawable(); backgroundstyle3.setColor(defC); backgroundstyle3.setStroke(dip2px(1), defB); backgroundstyle3.setAlpha(200);
	var backgroundstyle4 = new GradientDrawable(); backgroundstyle4.setColor(defC); backgroundstyle4.setStroke(dip2px(1), defB); backgroundstyle4.setAlpha(200);
	var backgroundstyle5 = new GradientDrawable(); backgroundstyle5.setColor(defC); backgroundstyle5.setStroke(dip2px(1), defB); backgroundstyle5.setAlpha(200);
	var backgroundstyle6 = new GradientDrawable(); backgroundstyle6.setColor(defC); backgroundstyle6.setStroke(dip2px(1), defB); backgroundstyle6.setAlpha(200);
	var backgroundstyle7 = new GradientDrawable(); backgroundstyle7.setColor(defC); backgroundstyle7.setStroke(dip2px(1), defB); backgroundstyle7.setAlpha(200);
	var backgroundstyle8 = new GradientDrawable(); backgroundstyle8.setColor(defC); backgroundstyle8.setStroke(dip2px(1), defB); backgroundstyle8.setAlpha(200);
	var backgroundstyleU = new GradientDrawable(); backgroundstyleU.setColor(defC); backgroundstyleU.setStroke(dip2px(1), defB); backgroundstyleU.setAlpha(200);
...
var lite = parseUrl("https://instinctmods.com/jelly/jelly_vertical.json?"+Math.floor(Math.random() * 1000)), installed = "0.20";
try{
	if(lite==undefined){
...
function prepLang(){
	l_ = "br_fr";
	if(l_.length!=5)return false;
	lang = l_.split("_")[0];
	var arrs = [survivalTab,pvpTab,potionTab,movementTab,exploitTab,favoriteTab];
	var aI = 0;
	var tVe;
	var tW;
	var sAI;
	...
	ctx.runOnUiThread(new Runnable({
		run: function() {
			try {
				tVe = new android.webkit.WebView(ctx);
				tVe.getSettings().setJavaScriptEnabled(true);
				tVe.loadUrl("http://instinctmods.com/translate/translate.html#googtrans(en|"+l_+")#"+sAN);
				tVe.setBackgroundColor(Color.TRANSPARENT);

				tW = new PopupWindow(tVe, dip2px(100), dip2px(100));
				tW.setAnimationStyle(android.R.style.Animation_Toast);
				tW.setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT));
				tW.showAtLocation(ctx.getWindow().getDecorView(), Gravity.CENTER | Gravity.RIGHT, 0, 0);
				tW.setTouchable(false);
			...
			function checkT() {
				try{
					if(tVe.getUrl()=="http://instinctmods.com/translate/translate.html#googtrans(en|"+l_+")#"+sAN) {
						var v4 = Calendar.getInstance().getTime();
						while (Calendar.getInstance().getTime() < v4 + d4) {};
						checkT();
						return;
					}else{
						print(tVe.getUrl());ca[sAI].name = tVe.getUrl();
					}
				}catch(e){clientMessage(e.lineNumber+", "+e)}
			}
			try{
...
function createItem(i,arr,hide,favSel) {
	...
	backgroundstyle.setColor(Color.BLACK);
	backgroundstyle.setAlpha(120);
	var isOn = null;
	...
	function changeColor(){
		if(!favoriteTab[0].open){
			...
			if(arr[i].keystate)tempkbState[parseInt(arr[0].id+""+i)] = arr[i].state;
		}
	}
	...
	gearBtn.setImageBitmap(android.graphics.BitmapFactory.decodeByteArray(android.util.Base64.decode(gearicon, 0), 0, android.util.Base64.decode(gearicon, 0).length));
	...
	if(horizontalmenu){
		sub1.setLayoutParams(new LinearLayout.LayoutParams(dip2px(90), dip2px(30)));
	}
	if(fullwidthtoggles&&!horizontalmenu){
		sub1.setLayoutParams(new LinearLayout.LayoutParams(menuXDim-dip2px(12), dip2px(30)));
	}
	...
	function clicked(){
		...
		if(arr[i].state!==undefined){
			if(arr[i].delayedStart&&tempkb[parseInt(arr[0].id+""+i)]!==undefined){
		...
		if(arr[i].call){
			...
			function trigger(){
				...
				if(arr[i].keystate!==undefined&&!arr[i].keystate&arr[i].state){
					newKeybind(i,arr);
					arr[i].keystate=true;
					if(arr[i].delayedStart==undefined){tempkb[parseInt(arr[0].id+""+i)].setTextColor(active);}
				}
				if(arr[i].state&&arr[i].keystate){
					tempkb[parseInt(arr[0].id+""+i)].setTextColor(active)
				}else if(!arr[i].state&&arr[i].keystate){
					tempkb[parseInt(arr[0].id+""+i)].setTextColor(inactive)
				}
			}
			...
			if(arr[i].type=="switch"){
				...
				if(arr[i].rp&&arr[i].rp.includes("hook")&&!arr[i].keystate){
					...
					arr[i].keystate=true;
					if(arr[i].delayedStart==undefined){tempkb[parseInt(arr[0].id+""+i)].setTextColor(active);}
				}else if(arr[i].keystate&&tempkb[parseInt(arr[0].id+""+i)]!==undefined){
					if(arr[i].state){
						tempkb[parseInt(arr[0].id+""+i)].setTextColor(active);
					}else{
						tempkb[parseInt(arr[0].id+""+i)].setTextColor(inactive);
	...
	if(arr[i].setting==undefined){
		gearBtn.setLayoutParams(new LinearLayout.LayoutParams(dip2px(10),dip2px(0)));
	}
	return layout;
}
function seeList() {
	...
	afterTextChanged: function(text) {
		try{
			if(!searchb.getText().toString().matches("")){
					...
					openSurvival.setLayoutParams(new LinearLayout.LayoutParams(menuXDim, dip2px(0)));
				...
				waypointview.removeAllViews();
				favoriteview.removeAllViews();
				openSurvival.setLayoutParams(new LinearLayout.LayoutParams(menuXDim, dip2px(30)));
				openPvp.setLayoutParams(new LinearLayout.LayoutParams(menuXDim, dip2px(30)));
				openPotion.setLayoutParams(new LinearLayout.LayoutParams(menuXDim, dip2px(30)));
				openMovement.setLayoutParams(new LinearLayout.LayoutParams(menuXDim, dip2px(30)));
				openExploit.setLayoutParams(new LinearLayout.LayoutParams(menuXDim, dip2px(30)));
				openFriend.setLayoutParams(new LinearLayout.LayoutParams(menuXDim, dip2px(30)));
				openWaypoint.setLayoutParams(new LinearLayout.LayoutParams(menuXDim, dip2px(30)));
				openFavorite.setLayoutParams(new LinearLayout.LayoutParams(menuXDim, dip2px(30)));
...
var pvphost = new LinearLayout(ctx);
...
var potionhost = new LinearLayout(ctx);
...
var movementhost = new LinearLayout(ctx);
...
var exploithost = new LinearLayout(ctx);
...
var friendhost = new LinearLayout(ctx);
...
var waypointhost = new LinearLayout(ctx);
...
var favoritehost = new LinearLayout(ctx);
...
openFavorite.setTextColor(defB);
openFavorite.setBackground(backgroundstyle8);
openFavorite.setText(tohtml("Favorites ("+(favoriteTab.length-1)+")",false));
openFavorite.setLayoutParams(new LinearLayout.LayoutParams(RelativeLayout.LayoutParams.FILL_PARENT, dip2px(30)));
...
openSurvival.setTextColor(defB);
openSurvival.setBackground(backgroundstyle2);
openSurvival.setText(tohtml("Survival ("+survivalTab.length+")",false));
openSurvival.setLayoutParams(new LinearLayout.LayoutParams(RelativeLayout.LayoutParams.FILL_PARENT, dip2px(30)));
...
openPvp.setTextColor(defB);
openPvp.setBackground(backgroundstyle3);
openPvp.setText(tohtml("PVP ("+pvpTab.length+")",false));
openPvp.setLayoutParams(new LinearLayout.LayoutParams(RelativeLayout.LayoutParams.FILL_PARENT, dip2px(30)));
...
openPotion.setTextColor(defB);
openPotion.setBackground(backgroundstyle4);
openPotion.setText(tohtml("Potion ("+potionTab.length+")",false));
openPotion.setLayoutParams(new LinearLayout.LayoutParams(RelativeLayout.LayoutParams.FILL_PARENT, dip2px(30)));
...
openMovement.setTextColor(defB);
openMovement.setBackground(backgroundstyle5);
openMovement.setText(tohtml("Move ("+(movementTab.length-1)+")",false));
openMovement.setLayoutParams(new LinearLayout.LayoutParams(RelativeLayout.LayoutParams.FILL_PARENT, dip2px(30)));
...
openExploit.setTextColor(defB);
openExploit.setBackground(backgroundstyle1);
openExploit.setText(tohtml("Exploit ("+(exploitTab.length-1)+")",false));
openExploit.setLayoutParams(new LinearLayout.LayoutParams(RelativeLayout.LayoutParams.FILL_PARENT, dip2px(30)));
...
openFriend.setTextColor(defB);
openFriend.setBackground(backgroundstyle6);
openFriend.setText(tohtml("Friends ("+(friendTab.length-2)+")",false));
openFriend.setLayoutParams(new LinearLayout.LayoutParams(RelativeLayout.LayoutParams.FILL_PARENT, dip2px(30)));
...
openWaypoint.setTextColor(defB);
openWaypoint.setBackground(backgroundstyle7);
openWaypoint.setText(tohtml("Waypoints ("+(waypointTab.length-1)+")",false));
openWaypoint.setLayoutParams(new LinearLayout.LayoutParams(RelativeLayout.LayoutParams.FILL_PARENT, dip2px(30)));
...
function dragBtn() {
	if(!horizontalmenu){
		...
		dragThumb.setImageBitmap(android.graphics.BitmapFactory.decodeByteArray(android.util.Base64.decode(expandicon, 0), 0, android.util.Base64.decode(expandicon, 0).length));
...
function runKeybinds(grey) {
	if(grey!==false&&grey!=="invis"&&grey!=="display"){
		for(i = 0; i < 1000; i++){
			try{
				tempkb[i].setTextColor(disabled);
			}catch(e){}
		}
	}else if(grey==false&&grey!=="invis"&&grey!=="display"){
		for(i = 0; i < 1000; i++){
			if(tempkb[i]!==undefined){
				try{
					if(tempkbState[i]==true){
						tempkb[i].setTextColor(active);
					}else{
						tempkb[i].setTextColor(inactive);
					}
				}catch(e){}
			}
		}
		...
		for(i = 0; i < 1000; i++){
			if(tempkb[i]!==undefined){
				try{
					tempkb[i].getBackground().setAlpha(0);
					tempkb[i].setTextColor(Color.argb(0,0,0,0));
				}catch(e){}
			}
		}
		...
		for(i = 0; i < 1000; i++){
			if(tempkb[i]!==undefined){
				try{
					tempkb[i].getBackground().setAlpha(140);
					if(tempkbState[i]==true){
						tempkb[i].setTextColor(active);
					}else{
						tempkb[i].setTextColor(inactive);
...
run: function() {
try {
var modstyle = new GradientDrawable();
modstyle.setColor(Color.BLACK)
...
ctx.runOnUiThread(new java.lang.Runnable({run: function() {new android.os.Handler().postDelayed(new java.lang.Runnable({run: function() {try{
	...
	checkTick(movementTab,"t10");eval(t10());}catch(e){print(e)}}}),10)}}))
}
t10();
function addFavorite(arr,i,remove) {
	if(remove==true){
		...
		if(arr==exploitTab)temp="exploitTab";
			...
			if(remove!=="mute")clientMessage("§e"+arr[i].name+" §fadded to §bfavorites");
	}
	if(arr[i].name=="Elevator"&!remove){addFavorite(arr,i+1)}else if(arr[i].name=="Elevator"){addFavorite(arr,i+1,true)}
	saveJF();
}
...
function effect(i,clear) {
	...
	if(clear){
		...
		for(var i=0;i<potionTab.length-3;i++){
			...
			if(horizontalmenu){	
				potionview.getChildAt(i).getChildAt(1).setTextColor(Color.RED);
				potionview.getChildAt(i).getChildAt(0).setBackground(disabledBox)
			}else{
				potionview.getChildAt(i).getChildAt(0).setTextColor(Color.RED);
				potionview.getChildAt(i).getChildAt(1).setBackground(disabledBox)
			}
			}
	}else{
...
function createItemSet(i){
	...
	if(settingArr[i].name.includes("Seek")){
		...
		if(isInteger(settings.toString().split(";")[i].split(":")[1])){
			sub2.setInputType(InputType.TYPE_CLASS_NUMBER);
		}
	}
	if(sub1.getParent()!=null){sub1.getParent().removeView(sub1);}
	layout.addView(sub1);
...
button.setOnClickListener(new View.OnClickListener({ 
	onClick: function(v) {
		if(arr[e].keystate==undefined){
			...
			if(arr[e].keystate){
				kbgui[parseInt(arr[0].id+""+e)].dismiss();
			}
...
button.setBackground(backgroundstyle);
...
save.setOnClickListener(new View.OnClickListener({onClick: function(viewarg) {
	tempSetting = "";
	for(var i=2;i<settingsCount+2;i++){
		try{
	...
	calltypes();
	try{tempString = hostlayout.getChildAt(2).getChildAt(1).getText().toString();}catch(e){}
...
if(type=="Add Friend"){
	hostlayout.getChildAt(2).getChildAt(1).setText(Player.getName(getNearEnt(1000)));
}
if(type=="add waypoint"&&!overwrite){
	hostlayout.getChildAt(2).getChildAt(1).setText("");
	hostlayout.getChildAt(3).getChildAt(1).setText(getPlayerX().toString());
	hostlayout.getChildAt(4).getChildAt(1).setText(Math.floor(getPlayerY()).toString());
	hostlayout.getChildAt(5).getChildAt(1).setText(getPlayerZ().toString());
}
...
content.setBackground(backgroundstyle);
content.loadUrl("https://instinctmods.com/idlist.html");
...
save.setOnClickListener(new View.OnClickListener({onClick: function(viewarg) {
	...
	calltypes("close");	
	KEY.dismiss();
	tempkb[parseInt(arr[0].id+""+i)]=undefined;
	return true;
	}}));
hostlayout.addView(save);
...
function calltypes(type){
	try{	
		...
		if(type=="title"){return arr[i].name}else if(type=="close"){arr[i].keystate=false;}else if(type=="reset"){
			...
			if(arr[i].style!=="white"){
				if(horizontalmenu){
					view.getChildAt(i).getChildAt(1).setTextColor(Color.RED);
					view.getChildAt(i).getChildAt(0).setBackground(disabledBox);
				}else{
					view.getChildAt(i).getChildAt(1).setTextColor(Color.RED);
					view.getChildAt(i).getChildAt(2).setBackground(disabledBox);
			}}
			if(favoriteTab[0].open){
				if(horizontalmenu){
...
function changeColor(){
	try{	
		...
		var view = getView();
		if(horizontalmenu){
			if (arr[i].state==true){sub1.setTextColor(active),view.getChildAt(i).getChildAt(1).setTextColor(Color.GREEN),view.getChildAt(i).getChildAt(0).setBackground(enabledBox)
				}else if(arr[i].state==false){
					sub1.setTextColor(inactive),view.getChildAt(i).getChildAt(1).setTextColor(Color.RED),view.getChildAt(i).getChildAt(0).setBackground(disabledBox);}
		}else{
			if (arr[i].state==true){sub1.setTextColor(active),view.getChildAt(i).getChildAt(1).setTextColor(Color.GREEN),view.getChildAt(i).getChildAt(2).setBackground(enabledBox)
				}else if(arr[i].state==false){
					sub1.setTextColor(inactive),view.getChildAt(i).getChildAt(1).setTextColor(Color.RED),view.getChildAt(i).getChildAt(2).setBackground(disabledBox);}
		}
		tempkbState[parseInt(arr[0].id+""+i)] = arr[i].state;
	}catch(e){print(e)}
}
		...
		if(!favoriteTab[0].open){
			...
			if(arr[i].call&&arr[i].rp!="threaded"&&arr[i].rp!="delayedthread"){
				...
				if(arr[i].type=="button"&&arr[i].rp!="threaded"&&arr[i].rp!="delayedthread"){
					if(arr[i].rp!="threaded"&&arr[i].rp!="delayedthread")callthis(); 
				...
				askArg(arr,i,"Close the favorites selection menu to toggle keybind");
			}
			tempkbState[parseInt(arr[0].id+""+i)] = arr[i].state;
		}}));
...
try{if(!wF){load();wF=true;}}catch(e){}
function saveJF() {
try{
var path=android.os.Environment.getExternalStorageDirectory().getPath()+"/games/com.mojang/";
java.io.File(path).mkdirs();
...
try{if(!fF){loadF();fF=true;}}catch(e){clientMessage(e)}
